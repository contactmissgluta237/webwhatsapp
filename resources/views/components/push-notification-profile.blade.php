        ...
        <div class="row">
            <div class="col-md-8">
                <div id="push-profile-actions">
                    <button id="profile-enable-push" 
                            class="btn btn-primary mr-2" 
                            onclick="profileEnablePush()"
                            style="display: none;">
                        <i class="ft-bell mr-1"></i>
                        Activer les notifications
                    </button>
                    
                    <button id="profile-disable-push" 
                            class="btn btn-outline-danger mr-2" 
                            onclick="profileDisablePush()"
                            style="display: none;">
                        <i class="ft-bell-off mr-1"></i>
                        Désactiver les notifications
                    </button>
                    
                    <button id="profile-test-push" 
                            class="btn btn-outline-secondary" 
                            onclick="profileTestPush()"
                            style="display: none;">
...
async function profileTestPush() {
    ...
    try {
        ...
        showPushProfileAlert('info', 'Test envoyé', 'Une notification de test a été envoyée. Vérifiez vos notifications.');
...
function showPushProfileAlert(type, title, message) {
    ...
    const alertClasses = {
        success: { alert: 'alert-success', icon: 'ft-check-circle text-success' },
        danger: { alert: 'alert-danger', icon: 'ft-x-circle text-danger' },
        warning: { alert: 'alert-warning', icon: 'ft-alert-triangle text-warning' },
        info: { alert: 'alert-info', icon: 'ft-info text-info' }
    };
...
async function updatePushProfileStatus() {
    ...
    try {
        if (!window.pushManager) {
            ...
            showPushProfileAlert('warning', 'Initialisation', 'Service de notifications en cours d\'initialisation...');
            return;
        }
        ...
        if (!isSupported) {
            ...
            disableBtn.style.display = 'none';
            testBtn.style.display = 'none';
            showPushProfileAlert('danger', 'Non supporté', supportInfo.reason + '. ' + (supportInfo.suggestion || ''));
        ...
        if (isSubscribed) {
            ...
            showPushProfileAlert('success', 'Activées', 'Les notifications push sont actives pour votre compte.');
        } else {
            enableBtn.style.display = 'inline-block';
            disableBtn.style.display = 'none';
            testBtn.style.display = 'none';
            enableBtn.disabled = false;
            showPushProfileAlert('info', 'Non activées', 'Activez les notifications pour recevoir les alertes importantes.');
        }
