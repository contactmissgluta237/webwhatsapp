<div class="dropdown" style="position: static;">
    ...
    <div class="dropdown-menu dropdown-menu-right" style="position: absolute; z-index: 1050; min-width: 220px;">
        ...
        {{-- Supprimer --}}
        <form method="POST" action="{{ route('whatsapp.destroy', $account->id) }}" style="display: inline; width: 100%;">
            @csrf
            @method('DELETE')
            <button type="submit" class="dropdown-item py-2 text-danger text-left border-0 bg-transparent w-100"
                    onclick="return confirm('Supprimer définitivement la session « {{ $account->session_name }} » ?')"
                    style="background: none !important;">
